$(document).ready(function() {
	...
	$('#data_table_matreq_hdr').DataTable({
		processing: true,
		serverSide: true,
		ajax: {
			url: 'material_request/list_material_request_header',
			type: 'GET'
		},
	...
	$('#data_table_matreq_hdr_wrapper').removeClass('container-fluid');
	...
	function reset_input() {
		...
		$('.save_matreq_hdr').prop('disabled', false);
		$('.cancel_matreq_hdr').prop('disabled', false);
	}
	
	$('#new_matreq_hdr').click(function () {
		reset_input();
		reset_error();
		$('#state').val('ADD');
		$('#modal_title_matreq_hdr').html("Add New Material Request");		
		$('#modal_matreq_hdr').modal('show');
	});

	$('.save_matreq_hdr').click(function(e) {
		e.preventDefault();
		var loading;
		...
		if (state == 'ADD') {
			...
			url = 'material_request/'+id;
			tipe = 'PUT';
		}
		
		$('.save_matreq_hdr').prop('disabled', true);
		$('.cancel_matreq_hdr').prop('disabled', true);
		
		$.ajax({
			type: tipe,
			url: url,
			data: $('#modal_form_matreq_hdr').serialize(),
			beforeSend: function() {
				loading = new Loading(
					{
						direction: 'hor',
			...
			success: function(data) {
				if ($.isEmptyObject(data.errors)) {
					$('#modal_matreq_hdr').modal('hide');
					var oTableMaterialRequestHdr = $('#data_table_matreq_hdr').dataTable();
					oTableMaterialRequestHdr.fnDraw(false);
					swal({
						title: "Success!",
					...
					window.location.href = "material_request/d/"+redirect;									
				} else {
					reset_error();
	...
	$('body').on('click', '#delete_matreq_hdr', function (e) {
		var id = $(this).data('id');
		var loading;
		swal({
			title: 'Are you sure?',
	...
	$('body').on('click', '#edit_matreq_hdr', function (e) {
		e.preventDefault();
		var loading;
		reset_input();
		reset_error();
		var id = $(this).data('id');
		var url = 'material_request/'+id+'/edit';
		$.ajax({
			url: url,
			type: 'GET',
			datatype: 'json',
			...
			success: function(data) {	
				...
				$('#state').val('EDIT');
				$('#modal_title_matreq_hdr').html("Edit Material Request");
				$('#modal_matreq_hdr').modal('show');
			}
		});
	});
